...
TEST(FactoryTest, CreateFromStream) {
    // Test Slaver
    {
        std::stringstream ss("1 100 200"); // Type=1 (Slaver), x=100, y=200
        auto npc = factory(ss);
    ...
    // Test Orc
    {
        std::stringstream ss("2 150 250"); // Type=2 (Orc), x=150, y=250
        auto npc = factory(ss);
        ASSERT_NE(npc, nullptr);
    ...
    // Test Druid
    {
        std::stringstream ss("3 300 400"); // Type=3 (Druid), x=300, y=400
        auto npc = factory(ss);
        ASSERT_NE(npc, nullptr);
...
TEST(FactoryTest, InvalidStreamInput) {
    ...
    // Неполные данные
    {
        std::stringstream ss("1 100"); // Только тип и x, нет y
...
TEST(FactoryTest, NPCMethodsAfterCreation) {
    ...
    auto orc = factory(OrcType, 150, 250);
    ...
    EXPECT_TRUE(slaver->is_close(*orc, 100)); // Расстояние ~71 < 100
    EXPECT_FALSE(slaver->is_close(*druid, 100)); // Расстояние ~361 > 100
}
